...
def adminauthverified(request, pk):
    ...
    return  redirect('admin', pk=ex.uuid)
def alldisptranapprovedpersona(request, pk, cc):
    userc = deposite.objects.get(uuid = pk)
    userc.approved = False
    s = Client.objects.filter(deposite__uuid = pk)
    gdgd = Client.objects.filter(deposite__uuid = pk).first()
    
    userc.save()
    messages.info(request, 'Declined successfully ')
    return redirect('alltranpersonal' ,pk = gdgd.uuid)    
def allptrandeleteallpersonal  (request, pk, cc):
    userc = deposite.objects.get(uuid = pk)
    gdgd = Client.objects.filter(deposite__uuid = pk).first()
    userc.delete()
    messages.info(request, 'Deleted successfully ')
    return redirect('alltranpersonal' ,pk = gdgd.uuid)
def alltranpersonal(request,pk):
    if not request.user.is_superuser:
        return redirect('loginuser')
    usercd = Client.objects.get(user=request.user)
    depotrans = Client.objects.get(uuid=pk)
    usertran = depotrans.deposite.all()
    ...
    return render(request,'adminC/html/alltranspersnal.html',consc)
def allptranapprovedperson(request, pk, cc):
    userc = deposite.objects.get(uuid = pk)
    userc.approved = True
    s = Client.objects.filter(deposite__uuid = pk)
    sjj = Client.objects.filter(deposite__uuid = pk).first()
   

    userc.save()
    messages.info(request, 'Approved successfully ')

    return redirect('alltranpersonal' ,pk = sjj.uuid)
...
def home(request):
    ...
    return render(request,'cover/home.html',c)
def contact(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
...
def Coprate(request):
    ...
    return render(request,'cover/cop.html',c)
def imprint(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/imprint.html',c)
def legalresouce(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/legal.html',c)
def histrocal(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/histrocal.html',c)
def glorbalnte(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/glorbalnte.html',c)
def news(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/news.html',c)
def carrer(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/carrer.html',c)
def whtanext(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/whatnext.html',c)
def accessbil(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/accessbil.html',c)
def media(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/media.html',c)
def whtanext2(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/whatnext2.html',c)
def whtanext1(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/whatnext1.html',c)
def respo(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/reposiblity.html',c)
def datap(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/dataprotect.html',c)
def report(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/report.html',c)
...
def contact(request):
    ...
    return render(request,'cover/contact.html',c)
def contcopreativact(request):
    i = Site.objects.get(uuid=1)
    c = {
    'i':Site.objects.get(uuid=1),
    }
    return render(request,'cover/copreative.html',c)
def mailxs(request):
    i = Site.objects.get(uuid=1)
...
def registeruser(request):
    i = Site.objects.get(uuid=1)
    c = {
    'site':Site.objects.get(uuid=1),
    }
    if request.method =="POST":
        passwordx = request.POST['pass1']
        passwordx2 = request.POST['pass2']
        ...
        password1 = passwordx.replace(" ", "")
        password2 = passwordx2.replace(" ", "")
            
             
 
 
        if  User.objects.filter(username__icontains=username,email__icontains=email).exists():
            messages.info(request, 'username and password already exist')
            ...
            if password1 == password2:
                ...
                messages.info(request, 'An verification mail was sent to your email ')
                return redirect('registeruser')

            else:
                messages.info(request, "Password Does't match ")
                return redirect('registeruser')
    return render (request, 'cover/register.html',c)
...
def Loginuser(request):
    ...
    if True:
        if request.method =="POST":
            ...
            usernamex = request.POST['acountname']  
            emailx = request.POST['password']
            password = emailx.replace(" ", "")
            accountnum = usernamex.replace(" ", "")
            
            if Client.objects.filter(AccountNUm__icontains=accountnum,password__icontains=password,disabled=False, approved=True ).exists():
                user = Client.objects.get(AccountNUm__icontains=accountnum,password__icontains=password)
                alluser = User.objects.filter(username__icontains=user.user.username,email__icontains=user.user.email).first()

                authenticate(username=user.user.username, password=user.password)
                login(request, alluser)

                if alluser.is_superuser == True :
                        return redirect('admin' , pk = user.uuid )

                else:
                    if user.Pin:
                        return redirect('dashboard' , pk = user.uuid )
                    else:
                        return redirect('pin' , pk = user.uuid )
            elif Client.objects.filter(AccountNUm__icontains=accountnum,password__icontains=password, disabled=False, approved=False).exists():
                messages.error(request,'An verification mail was send to your email')
                user = Client.objects.get(AccountNUm__icontains=accountnum,password__icontains=password)
                message = f'http://{request.get_host()}/linktoactivate/{user.Tancode}/'
                subject = f' {i.name}Activation Email'
                c={
                                'a':user,
                                'm':message,
                                'site':i,
                ...
                email_sending(request,'mail/email.html',c,f"NOTIFICATION: {i.name} Activation of Email ",user.user.email.lower())
            elif Client.objects.filter(AccountNUm__icontains=accountnum,password__icontains=password, disabled=True, approved=True).exists():
                user = Client.objects.get(AccountNUm__icontains=accountnum,password__icontains=password)
                messages.error(request,'Account has been suspened temporary')
                acconu = messages.error(request,' <a class="text-blue-400" href="mailto:support@hamdichtrust.com">Contact throught Email Or support for information</a>') 
                 
                email_sending(request,'mail/blockedmail.html',c,f"Important: Account Restriction Notice",user.user.email)
                return redirect('loginuser')
            else:
    ...
    return render (request, 'cover/login.html',c)
...
def finialtransfer(request, pk):
    ...
    email_sending(request,'mail/otp.html',c,f"NOTIFICATION {i.name} OTP CODE ",userc.user.email.lower())
    if  Client.objects.filter(uuid =pk, disabled=True).exists():
        c = {
           'a':request.user,
           'w':i
